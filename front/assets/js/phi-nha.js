...
fetch("http://25.20.166.7:8080/lv0/roominfo", roomRequestOptions)
  ...
  .then((result) => {
    if (result.message == "Get room info successfully") {
      ...
      fetch("http://25.20.166.7:8080/lv0/housefee?id="+roomid, feeRequestOptions)
      .then((response) => response.json())
      .then((result) => {
        if (result.message == "Get vehicle fee success") {
          $("#house-feee").text(result.cc_fee.fee_cost);
          $("#house-fee").text(result.cc_fee.fee_cost);
          

          if(result.cc_fee.fee_status === "paid"){
